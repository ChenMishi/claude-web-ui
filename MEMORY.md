...
- **A. 定时任务（持久化）**：创建到 Web UI TimerDropdown 面板，关闭会话也继续执行，支持暂停/恢复/maxRuns自动停止/结果回显。
  ...
  - 方式2：生成 JWT（密钥文件路径 `~/.claude-web-ui/.jwt-secret`，用 `cat ~/.claude-web-ui/.jwt-secret` 读取），payload `{userId:"7f92a69b-46bb-4dd8-91f5-c5fdec7c8497",username:"admin",role:"admin"}`，调 `POST /api/scheduled-tasks`，body `{name,sessionId,command,interval(ms),maxRuns(可选)}`
